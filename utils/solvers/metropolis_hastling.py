...
def _metropolis_hastings_step_mexican_hat(
    spins: np.ndarray,
    J: float,
    T: float,
    positive_interaction_weight_kernel_sigma: float,
    negative_interaction_weight_kernel_sigma: float,
    kernel_size: int,
    spin_deviation_sigma: float,
) -> None:
    random_index = np.array([np.random.randint(dim_size) for dim_size in spins.shape])
    assert kernel_size % 2 != 0
    n_bound = int(kernel_size // 2)
    pos_interaction_kernel = get_gaussian_kernel(
        kernel_size, positive_interaction_weight_kernel_sigma
    )
    neg_interaction_kernel = get_gaussian_kernel(
        kernel_size, negative_interaction_weight_kernel_sigma
    )
    interaction_kernel = pos_interaction_kernel - neg_interaction_kernel
    picked = pick_neighbor(
        spins,
        center=random_index,
        distances=np.full(len(spins.shape), fill_value=n_bound),
...
def metropolis_hastings_mexican_hat(
    spins: np.ndarray,
    interaction_J: float = 1.0,
    temperature: float = 0.005,
    interaction_kernel_size: int = 17,
    pos_interaction_sigma: float = 5,
    neg_interaction_sigma: float = 5,
    spin_deviation_sigma: float = np.pi / 10,
    steps: int = 100000,
    save_callback: Callable | None = None,
) -> np.ndarray:
    ...
    i: int = 0
    while i < steps:
        _metropolis_hastings_step_mexican_hat(
            spins,
            interaction_J,
            temperature,
            pos_interaction_sigma,
            neg_interaction_sigma,
            interaction_kernel_size,
            spin_deviation_sigma,
        )
        if save_callback:
